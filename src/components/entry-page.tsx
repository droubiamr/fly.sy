  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <section>
        ...
        {e.note && <p className="mt-3 max-w-prose text-[17px] leading-relaxed">{e.note[locale]}</p>}
        <Provenance source={e.source} locale={locale} m={m} />
      </section>

      <section aria-labelledby="via-h">
        <h2 id="via-h" className="mb-3 text-xl font-bold">
          {m.entry.via} {e.name[locale]}
        </h2>
        {via.length === 0 ? (
          <p className="rounded-2xl border-[1.5px] bg-card p-5 text-lg text-muted-foreground">{m.entry.viaEmpty}</p>
        ) : (
              ...
              return (
                <li key={i} className="py-4">
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 text-[17px]">
                    <StatusStamp status={a.status} label={m.status[a.status]} size="sm" />
                    ...
                    <span className="ms-auto text-base text-muted-foreground">{formatHours(a.hours, locale)}</span>
                  </div>
                  {a.note && <p className="mt-1 text-base leading-relaxed">{a.note[locale]}</p>}
      ...
      {roads.length > 0 && (
        <section aria-labelledby="roads-h">
          <h2 id="roads-h" className="mb-2 text-xl font-bold">
            {m.entry.roads}
          </h2>
          <p className="mb-2 text-[15px] text-muted-foreground">{m.entry.roadsNote}</p>
          <ul className="grid grid-cols-2 gap-x-4 rounded-2xl border-[1.5px] bg-card px-5 sm:grid-cols-3">
            {roads.map(([cityId, h]) => {
              ...
              return (
                <li key={cityId} className="flex items-center justify-between border-b py-3 text-[17px] last:border-0">
                  {linked ? (
                  ...
                  <span className="text-base text-muted-foreground">{formatHours(h, locale)}</span>
                </li>
              )
            })}
          </ul>
      ...
      <section aria-labelledby="need-h">
        <h2 id="need-h" className="mb-3 text-xl font-bold">
          {m.entry.need}
        </h2>
        <div className="flex flex-col gap-3">
          {PASSPORTS.map((pp) => (
            <div key={pp} className="rounded-2xl border-[1.5px] bg-card px-5 py-4">
              <h3 className="text-lg font-bold">{m.reports.form.passports[pp]}</h3>
      ...
      <section aria-labelledby="rep-h">
        <h2 id="rep-h" className="mb-3 text-xl font-bold">
          {m.entry.reports}
        </h2>
        {reports.length === 0 ? (
          <p className="text-[17px] text-muted-foreground">{m.entry.reportsEmpty}</p>
        ) : (
          <ul className="divide-y rounded-2xl border-[1.5px] bg-card px-5">
            {reports.map((r) => {
              ...
              return (
                <li key={r.id} className="py-4">
                  ...
                  <p className="mt-1 text-[17px] leading-relaxed">{typeof r.note === "string" ? r.note : r.note[locale]}</p>
                </li>
              )
            })}
          </ul>
      ...
      {siblings.length > 0 && (
        <section aria-labelledby="sib-h">
          <h2 id="sib-h" className="mb-3 text-xl font-bold">
            {kind === "air" ? m.crossings.airports : m.crossings.more}
          </h2>
          <ul className="flex flex-wrap gap-2">
            {siblings.map(([sid, se]) => (
              <li key={sid}>
                <Link href={href(entryPath(sid))} className="inline-flex min-h-12 items-center gap-2 rounded-xl border-[1.5px] border-input bg-card px-4 text-base font-semibold">
                  <StatusStamp status={se.status} label={m.status[se.status]} size="sm" />
                  {se.name[locale]}
                </Link>
              </li>
            ))}
