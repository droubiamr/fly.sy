  ...
  return (
    <>
      ...
      <PageHero
        ...
        crumbs={crumbs}
        media={
          <span className="size-16 rounded-full bg-background p-2.5 ring-1 ring-foreground/10">
            <AirlineLogo code={code} />
          </span>
        }
        title={al.name[locale]}
      >
        <p className="mt-5 flex flex-wrap items-center justify-center gap-2 text-sm text-muted-foreground">
          {state !== "open" && <StatusBadge status={state} label={m.status[state]} solid />}
          {m.airlines.country} <CountryTag code={al.country} /> · <span className="font-mono text-xs">{code}</span>
        </p>
      </PageHero>

      <PageBody>
        <Section id="r-h" title={m.airlines.routes}>
          {hops.length === 0 ? (
            <p className={cn(SURFACE, "p-5 text-sm text-muted-foreground")}>{m.airlines.empty}</p>
          ) : (
                ...
                return (
                  <li key={i} className="py-4">
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                      ...
                      <span className="ms-auto text-sm font-semibold">{formatHours(h.hours, locale)}</span>
                    </div>
                    {h.note && <p className="mt-1.5 text-[13.5px] leading-relaxed">{h.note[locale]}</p>}
                    <Provenance confidence={h.confidence} source={h.source} seen={h.seen} locale={locale} m={m} />
                    <Link
                      href={href(routePath(o.id, destinationVia(h.entry)))}
        ...
        <Section id="o-h" title={m.airlines.more}>
          <ul className="flex flex-wrap gap-2">
            {others.map((c) => (
              <li key={c}>
                <Link href={href(airlinePath(c))} className={cn(CHIP, "ps-1.5")}>
                  <span className="size-8 shrink-0 rounded-full bg-background p-1 ring-1 ring-foreground/10">
                    <AirlineLogo code={c} />
                  </span>
